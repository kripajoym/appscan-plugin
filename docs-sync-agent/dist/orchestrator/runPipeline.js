...
async function main() {
    ...
    if (!impact.needs_update || targetFiles.length === 0) {
        console.log(JSON.stringify({ status: "no-op", commit, diffContext, analysis, impact, agentLifecycle: agentRuntime.getLifecycle() }, null, 2));
        return;
    }
    ...
    if (updatedFiles.length === 0) {
        console.log(JSON.stringify({ status: "no-op-after-review", commit, targetFiles, agentLifecycle: agentRuntime.getLifecycle() }, null, 2));
        return;
    }
