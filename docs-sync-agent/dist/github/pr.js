...
export async function createOrUpdatePullRequest(input) {
    const queryUrl = `${input.apiUrl}/repos/${input.owner}/${input.repo}/pulls?state=open&head=${encodeURIComponent(`${input.owner}:${input.draft.head}`)}&base=${encodeURIComponent(input.draft.base)}`;
    ...
    const pulls = JSON.parse(existing.text);
    if (pulls.length > 0) {
    ...
    return JSON.parse(created.text);
}
